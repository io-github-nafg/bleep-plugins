...
object ScoverageScript extends BleepScript("scoverage") {
  ...
  class CoverageApp(started: Started, bleepCommands: Commands) extends CommandsEntryPoint {
    ...
    object reportCommand extends Command[ReportOptions] {
      private lazy val (coverageDataDirs, coverageProjects) =
        started.build.explodedProjects
          .filterNot(_._2.isTestProject.contains(true))
          .keys
          .flatMap { name =>
            val dir = coverageDataDir(started.projectPaths(name))
            Option.when(Files.exists(dir))(dir -> name)
          }
          .unzip
      ...
      override def run(options: ReportOptions, remainingArgs: RemainingArgs): Unit = {
        ...
        CoverageAggregator.aggregate(coverageDataDirs.map(_.toFile).toList, started.buildPaths.buildDir.toFile) match {
          ...
          case Some(coverage) =>
            ...
            options.reporter.foreach { reporter =>
              ...
              reporter.write(coverage, coverageProjects.flatMap(started.projectPaths(_).sourcesDirs.all).toSeq, outDir)
            }
        }
      }
    }
